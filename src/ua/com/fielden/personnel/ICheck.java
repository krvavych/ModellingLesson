...
public interface ICheck<T> {
	public static <T> Result choose(final T o1, final T o2,
			final Set<Integer> chosen) throws IllegalArgumentException,
			IllegalAccessException {
		final Set<String> road = new HashSet<>();
		final Set<Set<Object>> compared = new HashSet<>();
		return deepEqualsNext(o1, o2, compared, chosen, road);
	}

	static <T> Result deepEqualsNext(final T object1, final T object2,
			final Set<Set<Object>> compared, final Set<Integer> chosen, final Set<String> road)
		...
		for (int fieldNum = 0; fieldNum < fieldsOfObject1.length; fieldNum++) {
			if (chosen.contains(fieldNum)) {
				fieldsOfObject1[fieldNum].setAccessible(true);
				fieldsOfObject2[fieldNum].setAccessible(true);
				final Object newObject1 = fieldsOfObject1[fieldNum]
						.get(object1);
				final Object newObject2 = fieldsOfObject2[fieldNum]
						.get(object2);
